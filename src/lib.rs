    ...
    if let Ok(bytes_read) = stream.read(&mut size_buffer) {
        match bytes_read {
            2 => {
                ...
                let msg: protobuf::WatchfsCommand =
                    protobuf::WatchfsCommand::decode(&mut bytes_array).unwrap();
                stop = msg.stop;
            }
                        ...
                        match event {
                            DebouncedEvent::NoticeWrite(path) => {
                                ...
                                debounced_event.action =
                                    protobuf::debounced_event::Action::NoticeWrite as i32;
                            }
                            DebouncedEvent::NoticeRemove(path) => {
                                debounced_event.path =
                                    Some(path.into_os_string().into_string().unwrap());
                                debounced_event.action =
                                    protobuf::debounced_event::Action::NoticeRemove as i32;
                            }
                            DebouncedEvent::Create(path) => {
                                debounced_event.path =
                                    Some(path.into_os_string().into_string().unwrap());
                                debounced_event.action =
                                    protobuf::debounced_event::Action::Create as i32;
                            }
                            DebouncedEvent::Write(path) => {
                                debounced_event.path =
                                    Some(path.into_os_string().into_string().unwrap());
                                debounced_event.action =
                                    protobuf::debounced_event::Action::Write as i32;
                            }
                            DebouncedEvent::Chmod(path) => {
                                debounced_event.path =
                                    Some(path.into_os_string().into_string().unwrap());
                                debounced_event.action =
                                    protobuf::debounced_event::Action::Chmod as i32;
                            }
                            DebouncedEvent::Remove(path) => {
                                debounced_event.path =
                                    Some(path.into_os_string().into_string().unwrap());
                                debounced_event.action =
                                    protobuf::debounced_event::Action::Remove as i32;
                            }
                            DebouncedEvent::Rename(path1, path2) => {
                                debounced_event.path =
                                    Some(path1.into_os_string().into_string().unwrap());
                                debounced_event.action =
                                    protobuf::debounced_event::Action::Rename as i32;
                            }
                            DebouncedEvent::Rescan => {
                                debounced_event.path = None;
                                debounced_event.action =
                                    protobuf::debounced_event::Action::Rescan as i32;
                            }
